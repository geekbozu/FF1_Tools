...
OFS = 0x80 + (NumFiles*0x80)

previousFileSize = 0
for i in range(len(FileMetaData)):
    FileMetaData[i][2]=OFS+previousFileSize
    OFS = FileMetaData[i][2]
    previousFileSize = FileMetaData[i][1]
    FullSize = FileMetaData[i][2]+previousFileSize
...
#    array('L',FullSize).toFile(f)
    ...
    for i in range(len(FileMetaData)):
        saveOFS = f.tell()
        f.write(FileMetaData[i][0])
        f.seek(saveOFS)
        f.seek(0x16,1)
        f.write(pack('H',int(FileMetaData[i][3])))
        f.write(pack('L',FileMetaData[i][2]))
        f.write(pack('L',FileMetaData[i][1]))
        f.write(pack('L',FileMetaData[i][1]))
        f.seek(0x5C,1)


    for i in range(len(FileMetaData)):
        with open('{}/{}-{}'.format(args.Input,FileMetaData[i][3],FileMetaData[i][0]),'rb') as t:
                f.write(t.read())
